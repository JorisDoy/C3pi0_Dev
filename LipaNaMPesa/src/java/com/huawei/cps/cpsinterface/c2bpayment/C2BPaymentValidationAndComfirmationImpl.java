        ...
        if (sel.isTransactionIdValid(datasource, c2BPaymenConfirmationRequest.getTransID())) {
            if (recharge(Configs.PRETUPSURLRECHARGE, msisdn.replaceAll("^254", "0"), c2BPaymenConfirmationRequest.getTransAmount())) {
                ...
                ins.insertConfirmationData(datasource, c2BPaymenConfirmationRequest.getTransType(), c2BPaymenConfirmationRequest.getTransID(),
                        c2BPaymenConfirmationRequest.getTransTime(), c2BPaymenConfirmationRequest.getTransAmount(), c2BPaymenConfirmationRequest.getBusinessShortCode(),
                        msisdn, c2BPaymenConfirmationRequest.getInvoiceNumber(), c2BPaymenConfirmationRequest.getOrgAccountBalance(),
                        c2BPaymenConfirmationRequest.getThirdPartyTransID(), c2BPaymenConfirmationRequest.getMSISDN(), "Replace with KYC", Configs.TRANSACTSTATUS_FAILED + " " + ejaze_message, Configs.ISVALIDATEDTRUE, c2BPaymenConfirmationRequest.getThirdPartyTransID());
...
//                mailer.start();
            ...
            if (!recharge(Configs.PRETUPSURLRECHARGE, msisdn.replaceAll("^254", "0"), c2BPaymenConfirmationRequest.getTransAmount())) {
                ...
                ins.insertConfirmationData(datasource, c2BPaymenConfirmationRequest.getTransType(), c2BPaymenConfirmationRequest.getTransID(),
                        c2BPaymenConfirmationRequest.getTransTime(), c2BPaymenConfirmationRequest.getTransAmount(), c2BPaymenConfirmationRequest.getBusinessShortCode(),
                        msisdn, c2BPaymenConfirmationRequest.getInvoiceNumber(), c2BPaymenConfirmationRequest.getOrgAccountBalance(),
                        c2BPaymenConfirmationRequest.getThirdPartyTransID(), c2BPaymenConfirmationRequest.getMSISDN(), "Replace with KYC", Configs.TRANSACTSTATUS_FAILED + " " + ejaze_message, Configs.ISVALIDATEDFALSE, c2BPaymenConfirmationRequest.getThirdPartyTransID());
                logger.info("Failed to topup number: " + c2BPaymenConfirmationRequest.getBillRefNumber());
                 //** 
//                HashMap<String, String> receipients = new HashMap<>();
...
//                mailer.start();
                ...
                ins.insertConfirmationData(datasource, c2BPaymenConfirmationRequest.getTransType(), c2BPaymenConfirmationRequest.getTransID(),
                        c2BPaymenConfirmationRequest.getTransTime(), c2BPaymenConfirmationRequest.getTransAmount(), c2BPaymenConfirmationRequest.getBusinessShortCode(),
                        msisdn, c2BPaymenConfirmationRequest.getInvoiceNumber(), c2BPaymenConfirmationRequest.getOrgAccountBalance(),
                        c2BPaymenConfirmationRequest.getThirdPartyTransID(), c2BPaymenConfirmationRequest.getMSISDN(), "Replace with KYC", Configs.TRANSACTSTATUS_SUCCESSFUL, Configs.ISVALIDATEDFALSE, c2BPaymenConfirmationRequest.getThirdPartyTransID());
                logger.info("Top up number but it was not validated: " + c2BPaymenConfirmationRequest.getBillRefNumber());
            }
//            ins.insertConfirmationData(datasource, c2BPaymenConfirmationRequest.getTransType(), c2BPaymenConfirmationRequest.getTransID(),
//                    c2BPaymenConfirmationRequest.getTransTime(), c2BPaymenConfirmationRequest.getTransAmount(), c2BPaymenConfirmationRequest.getBusinessShortCode(),
//                    msisdn, c2BPaymenConfirmationRequest.getInvoiceNumber(), c2BPaymenConfirmationRequest.getOrgAccountBalance(),
...
//            logger.info("Top up number but it was not validated: " + c2BPaymenConfirmationRequest.getBillRefNumber());
    ...
    public boolean recharge(String url, String msisdn, String amount) {
        ...
        if (!recharge.isIsSuccessful()) {
            ...
            if (recharge.getTXNSTATUS() == null) {
                message = "Ejaze <strong><b>FAILED</b></strong> to respond. Following topup will fail:<br/><b>MSISDN </b>" + msisdn + "<br/><b>Amount </b>" + amount;
            } else {
                message = "Ejaze returned an error with status: " + recharge.getTXNSTATUS() + " message: " + recharge.getTXNMESSAGE() + " during recharge. Transaction for following will fail <br/>" + "Customer Number: " + msisdn + "<br/> Aitime purchase amount: " + amount + "<br/>";
                logger.error("Ejaze failed to respond or there was an error during recharge");
                logger.error("Ejaze result code " + recharge.getTXNSTATUS());
                logger.error("Ejaze message :" + recharge.getTXNMESSAGE());
            }
            sm = new MailMapper(receipients, message, props);
            mailer = new Thread(sm);
            mailer.start();
    ...
    public C2BPaymentValidationResult validateC2BPayment(C2BPaymentValidationRequest c2BPaymentValidationRequest) {
        ...
        String cnum = c2BPaymentValidationRequest.getBillRefNumber().trim().replaceAll("[^0-9]", "").replace("+", "");
...
//            }
        ...
        if (vi.isValidInput(cnum, "[0-9]*") && cnum.length() != 0 && (cnum.length() == 10 || cnum.length() == 12 || cnum.length() == 9)) {
            ...
            String msisdn = "";
            if (!cnum.startsWith("0") && !cnum.startsWith("254")) {
            ...
            if (status.get("flag").equals("1")) { // means failed to reliably determine msisdn profile
                ...
                if (status.get("pp").equals("1")) {
                    ...
                    switch (status.get("s")) {
                        ...
                        // Point of change
//			case "E":
//			      sendSMS(saf_msisdn, cnum + " cannot be topped up");
//			      ins.insertValidationData(datasource, c2BPaymentValidationRequest.getTransType(), c2BPaymentValidationRequest.getTransID(), c2BPaymentValidationRequest.getTransTime(), c2BPaymentValidationRequest.getTransAmount(), c2BPaymentValidationRequest.getBusinessShortCode(), msisdn/*c2BPaymentValidationRequest.getBillRefNumber()*/, c2BPaymentValidationRequest.getInvoiceNumber(), c2BPaymentValidationRequest.getMSISDN(), fname + " " + lname + " " + mname, trans_id, c2BPaymentValidationRequest.getTransTime(), Configs.TRANSACTSTATUS_FAILED, "Number in status E");
//			      return respond("4", "Number not provisioned yet", trans_id);
                        default:
                            // save to DB
